...
`
-The function deepClonePolyfill takes an object obj as input along with an optional visited parameter, which is initialized as a WeakMap. The visited WeakMap keeps track of visited objects to avoid circular references.

- The function checks if the input obj is either null or not an object. If so, it returns the object itself because primitive values like null, strings, numbers, etc., do not need to be cloned.

- It checks if the visited WeakMap already contains the obj. If it does, it means the object has already been cloned, so it returns the cloned object from the visited map.

- If the obj is not null, not a primitive, and not yet visited, the function proceeds to clone it. It initializes an empty array or object (clone) based on whether the obj is an array or a plain object.

- It then sets the mapping between the original obj and its clone in the visited WeakMap to keep track of it.

- Next, it iterates over all own enumerable properties of the obj using a for...in loop.

- For each property, it recursively calls deepClonePolyfill on the property value and assigns the cloned value to the corresponding property in the clone.
